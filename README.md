...
| **Платёж по кредитке, внесённый раньше срока** | Банк присылает «внесите 989,84 ₽ до 30 сентября» и замолкает — подтверждения оплаты он не шлёт. Приложение само вычитает погашения, сделанные **после** напоминания: карта показывает «Платёж за этот период внесён», календарь перестаёт вычитать его из «Свободно», плитка на главной больше не подгоняет «платёж через 4 дня». Платёж, сделанный ДО напоминания, не засчитывается: банк уже знал о нём, когда считал свою цифру |
