    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            if (ownAccounts.isNotEmpty()) {
                Text("Счёт (необязательно)", style = FosType.SectionCap, color = FosColors.TextMuted)
                Text(
                    "Если указать, обязательство закроется только операцией с этого счёта — " +
                        "сопоставление станет точнее.",
                    style = FosType.Micro,
                    color = FosColors.TextMuted,
                )
                FlowRow(
                    ...
                    FilterChip(
                        ...
                        label    = { Text("Любой", style = FosType.Micro) },
                        shape    = RoundedCornerShape(FosDimens.RadiusChip),
                        colors   = chipColors(FosColors.Info),
                        border   = null,
                    )
                    ownAccounts.forEach { acc ->
                        FilterChip(
                            ...
                            label    = { Text(acc.name, style = FosType.Micro) },
                            shape    = RoundedCornerShape(FosDimens.RadiusChip),
                            colors   = chipColors(FosColors.Info),
                            border   = null,
                        )
