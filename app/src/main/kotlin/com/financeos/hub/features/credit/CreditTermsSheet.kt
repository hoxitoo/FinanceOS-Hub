 ...
 *
 * The term fields are the same composable the "new account" sheet uses, so a limit means the same
 * thing whether it was set at creation or edited later.
 */
...
fun CreditTermsSheet(
    ...
    onDismiss : () -> Unit,
    onSave    : (debtKopecks: Long, terms: CreditTermsState) -> Unit,
) {
    ...
    val terms = rememberCreditTermsState(account)

    val currencySymbol = FosFormatter.currencySymbol(account.currency)

    val dirty = {
        debtText != FosFormatter.amountInput(account.debtKopecks) || terms.differsFrom(account)
    }
    ...
    FosFormSheet(
        ...
        Button(
            onClick  = {
                onSave(FosFormatter.parseAmountInput(debtText) ?: 0L, terms)
                onDismiss()
            },
            modifier = Modifier.fillMaxWidth(),
