...
object MerchantNames {
    ...
    /**
     * Пришла ли первая часть ключа из [BRANDS] — то есть опознан ли сервис.
     *
     * У безымянной строки [groupKey] кладёт в ту же позицию ВЕСЬ очищенный текст, и внешне ключи
     * неразличимы: «chatgpt||openai» и «кофейня у дома||» устроены одинаково. Тому, кто сравнивает
     * ключи между собой, разница принципиальна — совпадение по распознанному бренду означает один
     * сервис, совпадение по случайному тексту не означает ничего.
     */
    fun isKnownBrand(token: String): Boolean =
        BRANDS.values.any { it.lowercase() == token }
