...
lastMatchedTxId: String?    ← main (largest) part — the one the event card shows
matchedTxIds: String?       ← all parts, comma-separated; null on rows written before v19
...
**Составной платёж.** Счёт за связь платят двумя переводами (телефон отдельно, интернет отдельно), и
по отдельности ни один из них не похож на обязательство: 550 ₽ и 1 500 ₽ против объявленных 2 000 ₽.
Поэтому после неудачи с одиночным совпадением матчер пробует сложить операции ОДНОГО календарного
дня. Ограничения тут и есть вся защита от жадности — без них две случайные покупки недели сложились
бы в нужную сумму без труда:

| Ограничение | Зачем |
|---|---|
| все части в один календарный день | счёт оплачивают за один присест, а не размазывают по неделе |
| не больше 3 частей | чем больше слагаемых, тем легче сойтись случайно |
| каждая ≥ 20 % от суммы | иначе копеечная покупка «добивала» бы 1 900 до 2 000 |
| не больше 8 кандидатов в дне | день с десятком подходящих трат — это обычный день, а не разбитый счёт |
| категории не противоречат | две РАЗНЫЕ известные категории — две разные траты; `null` не мешает |
| категория обязательства, если указана, есть среди частей | объявил «Связь» — части обязаны быть связью |

Одиночное совпадение всегда важнее составного. Из наборов берётся самый точный по сумме, при
равенстве — с наименьшим числом частей.

**Ручное «оплачено».** Осторожная эвристика обязана иметь выход: платёж наличными, перевод с чужой
карты, счёт, разбитый непохожим образом, приложение не увидит никогда. Кнопка на строке объявленного
платежа закрывает РОВНО эту дату (`matched_through`), не обязательство целиком, — следующий период
откроется как обычно, а «Отвязать» на той же строке возвращает всё назад. У платежа по кредитке и у
конца беспроцентного периода кнопки нет: эти даты приложение считает само, и «оплачено» там означало
бы спор с банком.
...
Связь односторонняя (`planned_payments.matched_tx_ids`, основная часть продублирована в
`last_matched_tx_id`), операция не меняется. Занятыми считаются ВСЕ части: оставь вторую свободной —
она закроет соседнее обязательство, и один платёж посчитается дважды. «Отвязать» записывает
`rejected_tx_id` — без этого следа сборщик тут же вернул бы ту же операцию на место.

Берётся БЛИЖАЙШАЯ к сроку операция, а не первая подходящая: список идёт по убыванию времени, и
«первая» означает «самая свежая».

## Правка операции: счёт, вторая сторона перевода, дата (`core/edit/`)

`TransactionEditor` — одна правка на экран «Операции» и лист на главной. Строка перечитывается перед
записью (инвариант #35). Чистые правила — в `TransactionEditRules`, закрыты тестами.

**Баланс** (`balanceAdjustments`). Строка «владеет» сдвигом, если у неё есть счёт, нет банковского
«Остатка» и она не из PDF (`balanceEffectOf`) — тот же критерий, что у удаления. Правка = откат
старого сдвига + новый сдвиг, с исключением: если у счёта ПОСЛЕ операции был банковский «Остаток»,
счёт заякорен, и сдвиги задним числом не делаются.

| Правка | Что с балансом |
|---|---|
| сирота → счёт без якоря | сумма операции ложится на счёт |
| сирота → счёт с «Остатком» позже | ничего: банк уже учёл эти деньги |
| счёт A → счёт B | A получает сумму обратно, B — теряет (каждый — если не заякорен) |
| счёт → без счёта | A получает сумму обратно |
| расход → доход на том же счёте | сдвиг на двойную сумму — исправление обратного знака |
| только дата | ничего |
| строка с «Остатком» → счёт | её «Остаток» становится снимком счёта, если свежее его правки |

Удаление использует то же правило якоря (`reversalFor`).

**Цель** (`goalPlan`). Пересчёт только когда сдвинулись деньги — счёт или сумма:

| Цель строки | Деньги сдвинулись | Итог |
|---|---|---|
| нет | да | привязка по новому счёту |
| нет | нет | ничего — старая операция не засчитывается в новую цель |
| по счёту | да | откат + привязка по новому счёту |
| по счёту | нет | остаётся |
| по карте/слову | ушла из перевода | откат, цель снята |
| по карте/слову | иначе | остаётся |

**Вторая сторона перевода** (`counterSide`): правится, если её нет или она записана приложением
(`source = MANUAL`). Сторона из маски в сообщении банка и отдельная банковская строка — только для
чтения (у второй своя карточка). Новая сторона — своя строка с общим `transferPairId`; её баланс —
по тому же правилу якоря; в цель она не засчитывается, если перевод уже засчитан по карте.

**Дата**: меняется день, время суток остаётся. У пары перевода обе ноги сдвигаются вместе.

**Валюта**: в выборе только счета в валюте операции.

## Платёж по кредитке: требование банка и его погашение

`duePayment` выбирает между цифрой банка и расчётом по циклу (см. инвариант #15) и **вычитает
погашения, сделанные после напоминания**.

| Поле `DuePayment` | Смысл |
|---|---|
| `amountKopecks` | сколько потребовали; не уменьшается, иначе «внесено 40 000 из 0 ₽» |
| `paidKopecks` | внесено после `accounts.due_payment_seen_at` |
| `remainingKopecks` | `amount − paid`, не ниже нуля — это и показывается, и вычитается из «Свободно» |
| `isSettled` | `amount > 0 && remaining == 0`: в этом периоде платить нечего |

Отсчёт от МОМЕНТА НАПОМИНАНИЯ (`due_payment_seen_at`), а не от начала месяца: банк считал свою цифру,
уже зная обо всём, что было раньше. При неизвестном моменте не засчитывается ничего — ошибки
несимметричны, лишний зачёт стоит человеку неустойки.

Погашение — ПОЛОЖИТЕЛЬНАЯ сумма (`repaidSince`), ровно как их считает `statementDueDebt`. Покупка
после напоминания обязательный платёж не уменьшает. Возврат покупки и кэшбэк при этом засчитываются
как погашение: они приходят тем же положительным движением и данных, чтобы их развести, в сообщении
банка нет — известное ограничение, общее со `statementDueDebt`.

Повторная доставка того же требования момент получения НЕ двигает (`noticeSeenAt`): SMS и пуш об
одном напоминании — две доставки одного события, а дедуп операций сюда не достаёт, потому что
вставки нет. «То же самое» — совпадение суммы и срока.

Расчётная ветка (`PaymentSource.INFERRED`) получает `paidKopecks = 0`: `statementDueDebt` уже нетто,
и второе вычитание закрыло бы выписку вдвое меньшим платежом.

Где это видно:
- **экран кредитки** — «Платёж за этот период внесён» вместо суммы, с приписками «банк просил X до
  Y · с тех пор внесено Z» и «долг по карте W — срок по нему ещё не наступил»; огранка карточки
  перестаёт быть тревожной, чип «через N дней» гаснет;
- **календарь** — событие несёт `remainingKopecks`, а `settled` получает второе условие
  `due.isSettled`. На 120-дневной карте работает только оно: `cycle == null`, и ветка «долг по
  выписке закрыт» не срабатывает;
- **плитка на главной** — срок приходит из `CalendarState.nextCreditDueInDays`. Сама главная считать
  его не может: она читает только текущий месяц, а платёж мог быть сделан до первого числа.

## Подписки: одно списание под разными именами (`core/analytics/SubscriptionDetector.kt`)

Группировка идёт по `MerchantNames.groupKey` — `бренд|уточнение|посредник`. Уточнение там не
случайно: оно разделяет РАЗНЫЕ продукты одного бренда («ADOBE *CREATIVE CLOUD» и «ADOBE *ACROBAT» —
две подписки, а не одна).

Обратная сторона обнаружилась на устройстве: один и тот же сервис банк присылает по-разному
(«ChatGPT», «VTS OPENAI *CHATGPT SUBSC», через разных посредников), ключи расходятся, и человек
видел **шесть подписок вместо трёх**, не понимая, за что платит дважды.

Спор разрешает ЦЕНА, а не текст: одна и та же подписка стоит одинаково, как её ни назови. Группы
одного бренда и одной валюты сливаются, если их типичные суммы совпадают. Три условия, и каждое
защищает от своей ошибки:

| Условие | Что было бы без него |
|---|---|
| допуск слияния **2 %**, а не общие 25 % | у человека ДВЕ подписки ChatGPT — 19,99 и 22,40; между ними 10,8 %, и общий допуск слил бы их в одну строку |
| бренд опознан (`MerchantNames.isKnownBrand`) | у безымянной строки в первой позиции ключа лежит весь очищенный текст — два разных магазина с одинаковым чеком слиплись бы в подписку |
| валюта совпадает | 19,99 $ и 19,99 ₽ — не одна подписка, а курса у офлайн-приложения нет |

Хостом становится группа с БОЛЬШИМ числом списаний: осколки присоединяются к обжитой группе, а не
наоборот. Известная и принятая цена решения: два аккаунта одного сервиса по ОДИНАКОВОЙ цене
покажутся одной строкой. Случай редкий, а обратная ошибка — дубликаты у каждого, кому банк меняет
описание, — была ежедневной.
