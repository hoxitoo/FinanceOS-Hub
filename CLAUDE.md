...
- [x] Календарь и «Свободно» — `features/calendar`, вход плиткой на главной под кредиткой.
      ...
      найденные подписки, дедлайны целей. Сетка месяца — следующим заходом.
...
# Next Steps
- **Календарь, заход 2** — сетка месяца вторым режимом рядом с полосой (полоса и список сделаны).
...
принимает ВЫЗЫВАЮЩИЙ, снаружи `item`.
