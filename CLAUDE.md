...
- Правка счёта не знает о ручной правке баланса, сделанной после операции: якорем считается только
  банковский «Остаток» (инвариант #39). Привязка сироты к счёту, баланс которого человек уже
  выправил руками, сдвинет его ещё раз.
...
### 39. Правка счёта задним числом двигает баланс — кроме случая, когда банк уже поставил свою цифру
...
Главный вопрос правки — **что делать с балансом**, и два существующих пути отвечали на него
противоположно: `reconcileAccount` усыновляет сирот БЕЗ дельты (надеясь на банковский «Остаток»),
а ручной ввод — С дельтой. Правило одно (`balanceAdjustments`): откатить старый сдвиг, применить
новый, **если только у счёта после этой операции не было банковского «Остатка»**. Тогда баланс
заякорен позже неё, в цифре банка эти деньги уже учтены, и дельта поверх уехала бы на сумму
операции. У Альфы «Остатка» в пушах нет — там дельта нужна, иначе привязанная операция не
отразилась бы на счёте вовсе.
- Якорь — ТОЛЬКО банковский «Остаток». Ручная правка баланса тоже якорит, но следа во времени не
  оставляет (`updatedAt` сдвигает каждая дельта) — известное ограничение.
- Один счёт решается ОДНИМ флагом, по времени старой строки: смена даты вместе со сменой знака иначе
  откатила бы старый сдвиг и не применила новый.
- Строка с «Остатком» и строка из PDF дельтой не двигают ничего — тот же критерий, что у удаления.
...
- **Удаление подчинено тому же якорю** (`TransactionEditor.reversalFor`). Иначе «отвязать счёт» и
  «удалить операцию» давали бы на одном счёте разные балансы: откат поверх банковской цифры.
- Смена ТИПА на том же счёте теперь двигает баланс: расход −1 500 → доход +1 500 — это +3 000. Раньше
  правка типа считалась «только для аналитики», и строка, записанная с обратным знаком (как было с
  СБП «от …»), чинилась в истории и оставалась испорченной на счёте.

Цель — одно правило (`goalPlan`) вместо `applyRetype`, который знал только про тип и удалён:
пересчёт только когда **сдвинулись деньги** (счёт или сумма). Правка названия не должна засчитать
старую операцию в цель, заведённую после неё (#31).
...
(был ли уже встречный пуш в ту минуту), а отдельная банковская строка правится в своей карточке.
Новая сторона — своя строка с общим `transferPairId` (#16). Одно событие пополняет цель один раз:
перевод, уже засчитанный по карте получателя, не засчитывается второй раз через новую ногу.

Список счетов в карточке — **только в валюте операции**: сумма пуша записана в его валюте, и привязка
рублёвой операции к долларовому счёту сдвинула бы баланс на 1 500 долларов. Выбор хранится как id
счёта, а не как пункт списка: пустой выбор из-за того, что счёта нет в списке, не должен означать
«отвязать» при простом «Сохранить».

Правка — **один `TransactionEditor` на оба экрана** и перечитывает строку перед записью (#35): у
главной и «Операций» были две копии, и они уже расходились (#30).
...
| Тесты | `app/src/test/kotlin/com/financeos/hub/` (34 файла, 434 случая) |
